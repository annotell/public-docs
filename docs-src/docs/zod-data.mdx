...
<Tabs>
  ...
  >
   Firsly we need to set up our KognicIOClient, if you have not set it up before checkout [quickstart](quickstart.mdx)
    ...
    def convert_zod_lidar_frame_to_point_cloud(lidar_frame, timestamp_ns: int) -> PointCloud:
    ...
    ```python
    from kognic.io.model import Image, ImageMetadata
    from zod.constants import Camera as ZodCamera

    def convert_zod_camera_frame_to_image(camera_frame) -> Image:
    ...
    and finally we can create the frames:
    ...
    for camera_frame, lidar_frame in list(sequence.info.get_camera_lidar_map())[:max_nr_frames]:
        ...
        # Ego pose
        # Ego pose is actaully just using the IMU data to 
        ...
        rot = R.from_matrix(ego_pose[:3, :3]).as_quat()
        ego_pose_converted = EgoVehiclePose(
          position=Position(x=ego_pose[0][3], y=ego_pose[1][3], z=extrinsic[2][3]),
          rotation=RotationQuaternion(w=rot[3], x=rot[0], y=rot[1], z=rot[2]))
        ...
        frames.append(
            LCSFrame(
                ...
                ego_vehicle_pose=ego_pose_converted,
