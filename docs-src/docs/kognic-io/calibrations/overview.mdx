...
# Creates a sample calibration for lidar.
...
# Creates a sample calibration for a Pinhole camera. For other camera types visit:
...
sensor_calibration = SensorCalibration(
  ...
  calibration={
    ...
    "RIGHT_FRONT_CAMERA": unity_pinhole_calibration()
  }
)
created_calibration = client.calibration.create_calibration(sensor_calibration)

# The retrieved calibration id is unique and should be referred
# to when creating scenes that use this calibration.
...
<details>
  <summary>
    <h2 style={{margin: '0px'}}>Listing calibrations</h2>
  </summary>
