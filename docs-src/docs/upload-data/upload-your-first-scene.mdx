...
You have successfully followed the [Quickstart](../getting-started/quickstart.mdx) guide and have the `kognic-io` library installed. For users with access to multiple workspaces you need to [specify a workspace to upload data too.](https://developers.kognic.com/docs/kognic-apis#specifying-a-workspace)
...
We have exemplar code and a tutorial for uploading scenes using [Zenseact Open Dataset (ZOD)](https://zod.zenseact.com/) data, including 2D, 3D,
...
check the status of the scene via the Python client or the Kognic Platform.
...
<Tabs>
    ...
    <TabItem value="platform" label="Kognic Platform" default>
        **If you do not have access to _Data Orchestration_, contact Kognic.**
...
You may see the scene initially in the _processing_ state while Kognic performs a little background processing, or in the
_ready_ or _failed_ state after processing completes.
